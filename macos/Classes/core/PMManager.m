...
- (void)getThumbWithId:(NSString *)id width:(NSUInteger)width height:(NSUInteger)height format:(NSUInteger)format quality:(NSUInteger)quality deliveryMode:(NSUInteger)deliveryMode resultHandler:(ResultHandler *)handler {
    ...
    if (entity && entity.phAsset) {
        ...
        [self fetchThumb:asset width:width height:height format:format quality:quality deliveryMode:deliveryMode resultHandler:handler];
    } else {
        [handler replyError:@"asset is not found"];
    }
}

- (void)fetchThumb:(PHAsset *)asset width:(NSUInteger)width height:(NSUInteger)height format:(NSUInteger)format quality:(NSUInteger)quality deliveryMode:(NSUInteger)deliveryMode resultHandler:(ResultHandler *)handler {
    ...
    [manager requestImageForAsset:asset
        ...
        FlutterStandardTypedData *data = [FlutterStandardTypedData typedDataWithBytes:resultData];
        [handler reply:data];
    }];
}
...
- (void)fetchFullSizeImageFile:(PHAsset *)asset
    ...
    [manager requestImageForAsset:asset
        ...
        NSString *path = [self writeFullFileWithAssetId:asset imageData:resultData];
